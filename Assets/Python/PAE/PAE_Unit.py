...
from CvPythonExtensions import (CyGlobalContext, CyInterface, CyMap, CyGame,
											...
											plotDistance, directionXYFromPlot, CyAudioGame, InterfaceDirtyBits)
...
def onUnitMoveOnSea(pUnit):
		...
		iPromo = gc.getInfoTypeForString("PROMOTION_ANGST_SEA")
		iPromoSave = gc.getInfoTypeForString("PROMOTION_RANG_NAVI_3")
		iCombatTypeNaval = gc.getInfoTypeForString("UNITCOMBAT_NAVAL")
		# Krake and Serpent gets PROMOTION_BEAST to disappear next turn (this promo is temporarily used for this)
		iPromoBeast = gc.getInfoTypeForString("PROMOTION_BEAST")
		LMonsterWillDisappear = [
			gc.getInfoTypeForString("UNIT_KRAKEN"),
			gc.getInfoTypeForString("UNIT_SEASERPENT")
		]
		if pUnit.getUnitType() in L.LUnitSeaMonsters:

			# Monster will disappear if discovered the turn before
			if pUnit.getUnitType() in LMonsterWillDisappear:
				if pUnit.isHasPromotion(iPromoBeast):
					pUnit.kill(True, -1)
					return

			# normal move on open sea
			iX = pPlot.getX()
			iY = pPlot.getY()
			for iI in range(DirectionTypes.NUM_DIRECTION_TYPES):
				loopPlot = plotDirection(iX, iY, DirectionTypes(iI))
				if not loopPlot.isNone():
					if loopPlot.isWater() and loopPlot.getNumUnits() > 0:
						if loopPlot.getUnit(0).getOwner() != pUnit.getOwner():
							for i in range(loopPlot.getNumUnits()):
								loopUnit = loopPlot.getUnit(i)
								if loopUnit.getUnitCombatType() == iCombatTypeNaval:
									if not loopUnit.isHasPromotion(iPromoSave) and not loopUnit.isHasPromotion(iPromo):
										loopUnit.setHasPromotion(iPromo, True)
										if gc.getPlayer(loopUnit.getOwner()).isHuman():
											iRand = 1 + CvUtil.myRandom(4, "SoundOfFearAtSeaMonsters")
											szSound = "AS2D_PAE_FEAR" + str(iRand)
											CyInterface().addMessage(loopUnit.getOwner(), True, 20, CyTranslator().getText("TXT_KEY_MESSAGE_SPOTTING_SEAMONSTER", ("",)),
												szSound, 2, pUnit.getButton(), ColorTypes(7), pPlot.getX(), pPlot.getY(), True, True)
								# Krake and Serpent
								if pUnit.getUnitType() in LMonsterWillDisappear:
									pUnit.setHasPromotion(iPromoBeast, True)
			return

		elif not pUnit.isHasPromotion(iPromoSave) and not pUnit.isHasPromotion(iPromo):
			iX = pPlot.getX()
			iY = pPlot.getY()
			for iI in range(DirectionTypes.NUM_DIRECTION_TYPES):
				loopPlot = plotDirection(iX, iY, DirectionTypes(iI))
				if not loopPlot.isNone():
					if loopPlot.isWater() and loopPlot.getNumUnits() > 0:
						loopUnit = loopPlot.getUnit(0)
						if loopUnit.getUnitType() in L.LUnitSeaMonsters:
							if loopUnit.getOwner() != pUnit.getOwner():
								pUnit.setHasPromotion(iPromo, True)
								if gc.getPlayer(pUnit.getOwner()).isHuman():
									iRand = 1 + CvUtil.myRandom(4, "SoundOfFearAtSeaMonsters")
									szSound = "AS2D_PAE_FEAR" + str(iRand)
									CyInterface().addMessage(pUnit.getOwner(), True, 20, CyTranslator().getText("TXT_KEY_MESSAGE_SPOTTING_SEAMONSTER", ("",)),
										szSound, 2, loopUnit.getButton(), ColorTypes(7), loopPlot.getX(), loopPlot.getY(), True, True)
								# Krake and Serpent
								if loopUnit.getUnitType() in LMonsterWillDisappear:
									loopUnit.setHasPromotion(iPromoBeast, True)
		# --------------------
		...
		# ------ Verletzte Schiffe bzw Cargo
		iDamage = pUnit.getDamage()
		# mit PAEVII in CvUnit::doTurn()
		#if iDamage > 10:
		...
		# PAE 7.15: ausgenommen Flottenkommandeure (Rangsystem für Schiffe)
		elif pPlot.isWater() and pPlot.getFeatureType() == -1 and not pPlot.isCity():
			if not pUnit.isHasPromotion(gc.getInfoTypeForString("PROMOTION_RANG_NAVI_4")):
...
# Init ab wieviel Einheiten der Stack versorgt werden muss
def getStackLimit():
...
def stackDoTurn(iPlayer, iGameTurn):
		...
		# es soll nur der Versorgungskarren groessere Armeen versorgen koennen und nicht die Heiler.
		...
		iStackLimit2 = iStackLimit1 * 3  # Rebellionschance
		if not pPlayer.isHuman():
				iStackLimit1 += 20
				iStackLimit2 += 20
		...
		while sUnit:
				...
				if not tmpPlot.isWater():
						...
						# Supply: PAE V: bei den Staedten gibts ne eigene funktion bei city supply
						if not tmpPlot.isCity():
								...
								if tmpA not in PlotArraySupply:
										...
										if tmpAnz >= 10:
												# AI Stack neben einer Stadt, in feindlichem Terrain (Ranged Strike)
												if not pPlayer.isHuman():
												...
												if tmpAnz >= iStackLimit1:
														...
														# wird weiter unten in PlotArraySupply neu berechnet
								...
								if pUnit.getOwner() == iPlayer:
										if pUnit.isRanged():
												if not pUnit.isMadeAttack() and pUnit.getImmobileTimer() <= 0:
														...
														# Ab 60% Schaden aufhoeren
														if pBestDefender.getDamage() < 60:
		...
		if PlotArraySupply:
				...
				for h in PlotArraySupply:
						...
						# Unit Suppy (-)
						iUnitSupply += iMelee
						if bDesert: iUnitSupply += iMounted * 2
						else: iUnitSupply += iMounted

						# Plot Supply (+)
						iPlotSupply = getSupplyFromPlot(iPlayer, loopPlot)

						# ***TEST***
						#CyInterface().addMessage(gc.getGame().getActivePlayer(), True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_TEST", ("Plot Supply", iPlotSupply)), None, 2, None, ColorTypes(10), 0, 0, False, False)
						...
						#CyInterface().addMessage(gc.getGame().getActivePlayer(), True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_TEST", ("Unit Supply Need", iUnitSupply)), None, 2, None, ColorTypes(10), 0, 0, False, False)


						iSupplyChange = iPlotSupply - iUnitSupply
						...
						if len(lHealer):
							for loopUnit in lHealer:
								...
								iMaxSupply = getMaxSupply(loopUnit)
								iSupplyHealer = getSupply(loopUnit)

								# ***TEST***
								#CyInterface().addMessage(gc.getGame().getActivePlayer(), True, 10, (u"Supply Wagon %d (max:%d): %d" % (loopUnit.getID(),iMaxSupply,iSupplyHealer)), None, 2, None, ColorTypes(10), 0, 0, False, False)

								# Healer kann sich aufladen
								if iSupplyChange > 0:

									if iSupplyHealer + iSupplyChange <= iMaxSupply:
										setSupply(loopUnit, iSupplyHealer + iSupplyChange)
										iSupplyChange = 0
									else:
										iSupplyChange -= (iMaxSupply - iSupplyHealer)
										setSupply(loopUnit, iMaxSupply)

								# Healer muss versorgen
								elif iSupplyChange < 0:

									if iSupplyHealer + iSupplyChange >= 0:
										setSupply(loopUnit, iSupplyHealer + iSupplyChange)
										iSupplyChange = 0
									else:
										iSupplyChange += iSupplyHealer
										setSupply(loopUnit, 0)
						...
						# ***TEST***
						#CyInterface().addMessage(gc.getGame().getActivePlayer(), True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_TEST",( "iSum", iSum)), None, 2, None, ColorTypes(10), 0, 0, False, False)


						if iSum > 0:
							# for Stack Rebellion
							# (iSum includes iStackLimit1)
							if iSum + iStackLimit1 > iStackLimit2:
							...
							for iUnit in range(iNumUnits):
									...
									if xDamage > 70 and not pPlayer.isHuman():
		...
		# +++++ Aufladen der restlichen Versorger (ausgenommen PlotArraySupply) UNIT_SUPPLY_WAGON ---------------------------------------
		for h in lHealerPlots:
			if h not in PlotArraySupply:
				loopPlot = gc.getMap().plot(h[0], h[1])
				iX = h[0]
				iY = h[1]

				# Healer Units on Plot
				lHealer = []
				...
				# Plot inits
				bFill = False
				iSupplyChange = getSupplyFromPlot(iPlayer, loopPlot)

				# ***TEST***
				#CyInterface().addMessage(gc.getGame().getActivePlayer(), True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_TEST", ("Healer only: Plot Supply", iSupplyChange)), None, 2, None, ColorTypes(10), 0, 0, False, False)


				# Inits for Supply Units (nur notwendig, wenns Versorger gibt)
				if iSupplyChange > 0 and len(lHealer) > 0:
						...
						# Fremdes Terrain
								...
								if iLoopOwner != -1:
										...
										# Versorger auf Vassalenterrain - Aufladechance - Stadt: 33%, Land 20%
										if pTeamPlot.isVassal(iTeam) and iSupplyChange > 0:
												if loopPlot.isCity() and CvUtil.myRandom(3, "SUPPLY_RELOAD_1_isCity") == 1:
														bFill = True
												elif CvUtil.myRandom(10, "SUPPLY_RELOAD_1") < 2:
														bFill = True

												if pPlayer.isHuman() and bFill:
										...
										# Versorger auf freundlichem Terrain - Aufladechance 20:x
												...
												if iAtt == 4:
														...
														iChance = 0
												if iChance > 0 and CvUtil.myRandom(20, "SUPPLY_RELOAD_2") < iChance:
														bFill = True
														if loopPlot.isCity():
		...
		# Healer Balken neuladen
		if lHealerPlots or PlotArraySupply:
...
def doUpgradeVeteran(pUnit, iNewUnit, bSetImmobile):
		if iNewUnit == -1:
				return
		...
		if pUnit is not None and not pUnit.isNone():
				...
				if pUnitOwner is not None and not pUnitOwner.isNone():
						...
						if bSetImmobile: NewUnit.setImmobileTimer(1)
...
def changeUnit(pUnit, iNewUnit, bSetImmobile):
		if pUnit.isNone() or iNewUnit == -1:
				return
		unitGroup = pUnit.getGroup()
		iNumUnits = unitGroup.getNumUnits()
		for i in range(iNumUnits):
				loopUnit = unitGroup.getUnitAt(i)
				doUpgradeVeteran(loopUnit, iNewUnit, bSetImmobile)
...
# Kein Check (notwendig für Helptext in CvGameUtils)
...
def doHunterHero(pWinner, pLoser):
		...
		if gc.getUnitInfo(pWinner.getUnitType()).getCombat() < gc.getUnitInfo(pLoser.getUnitType()).getCombat():
				if pLoser.getUnitType() == gc.getInfoTypeForString("UNIT_UR") or pLoser.getLevel() > 4 or pLoser.getUnitType() in L.LUnitSeaMonsters:
						if pLoser.isDead():
								if not pWinner.isHasPromotion(iPromoHero):
										pWinner.setHasPromotion(iPromoHero, True)
...
def doRankPromo(pWinner):
						...
						if iUnitType not in L.LUnitsHeadRang:
								if pWinnerPlayer.getCivilizationType() in L.LCivGermanen:
										if not pWinner.isHasPromotion(gc.getInfoTypeForString("PROMOTION_RANG_GER_3")):
												...
												CvUtil.addScriptData(pWinner, "P", "RangPromoUp")
								# Hunnen
								elif pWinnerPlayer.getCivilizationType() == gc.getInfoTypeForString("CIVILIZATION_HUNNEN"):
		...
		if pWinner.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_NAVAL"):
				if CvUtil.myRandom(5, "PROMOTION_RANG_NAVI") == 1:
					lPromos = [
						gc.getInfoTypeForString("PROMOTION_RANG_NAVI_1"),
						gc.getInfoTypeForString("PROMOTION_RANG_NAVI_2"),
						gc.getInfoTypeForString("PROMOTION_RANG_NAVI_3"),
						gc.getInfoTypeForString("PROMOTION_RANG_NAVI_4"),
						gc.getInfoTypeForString("PROMOTION_RANG_NAVI_5")
					]
					for iNewPromo in lPromos:
						if not pWinner.isHasPromotion(iNewPromo):
							pWinner.setHasPromotion(iNewPromo, True)
							# Der Kommandant Eurer Einheit (%s1) hat nun den Rang: %s2!
							if pWinnerPlayer.isHuman():
									CyInterface().addMessage(iWinnerPlayer, True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_UNIT_CIV_RANG", (pWinner.getName(), gc.getPromotionInfo(iNewPromo).getDescription())),
...
def flee(pLoser, pWinner, iWinnerDamage):
		...
		iMaxHealth = 5  # Standard: max 10% Gesundheit
				...
				if iRand < iChance:
						...
						if pLoserPlayer.isHuman():
								if bIsVillage:
										...
										CyInterface().addMessage(iLoserPlayer, True, 5, CyTranslator().getText("TXT_KEY_MESSAGE_UNIT_ESCAPE", (NewUnit.getName(), pWinner.getName())), None, 2, None, ColorTypes(8), 0, 0, False, False)
						if pWinnerPlayer.isHuman():
								if bIsVillage:
										CyInterface().addMessage(iWinnerPlayer, True, 5, CyTranslator().getText("TXT_KEY_MESSAGE_UNIT_ESCAPE_4", (NewUnit.getName(),)), None, 2, None, ColorTypes(13), 0, 0, False, False)
								elif bIsCity:
										CyInterface().addMessage(iWinnerPlayer, True, 5, CyTranslator().getText("TXT_KEY_MESSAGE_UNIT_ESCAPE_3", (NewUnit.getName(),)), None, 2, None, ColorTypes(13), 0, 0, False, False)
								else:
										CyInterface().addMessage(iWinnerPlayer, True, 5, CyTranslator().getText("TXT_KEY_MESSAGE_UNIT_ESCAPE_2", (NewUnit.getName(), pWinner.getName())), None, 2, None, ColorTypes(13), 0, 0, False, False)
...
def getMaxSupply(pUnit):
		iMaxSupply = 100 # Medic1 sollte die Einheit schon haben, die diese Funktion aufruft
		if pUnit.isHasPromotion(gc.getInfoTypeForString("PROMOTION_MEDIC2")): iMaxSupply += 100
		if pUnit.isHasPromotion(gc.getInfoTypeForString("PROMOTION_MEDIC3")): iMaxSupply += 100
		if pUnit.isHasPromotion(gc.getInfoTypeForString("PROMOTION_MEDIC4")): iMaxSupply += 100
		if pUnit.isHasPromotion(gc.getInfoTypeForString("PROMOTION_MEDIC5")): iMaxSupply += 100
...
def renegade(pWinner, pLoser):
		...
		if iWinnerUnitAIType in lUnitAI:
		...
		elif iWinnerUnitType in L.LUnitWildAnimals + L.LUnitSeaMonsters:
				bUnitRenegades = False
				...
				iLoserUnitType in L.LUnitSeaMonsters
				):
				bUnitRenegades = False
		...
		if bUnitRenegades:
				...
				iUnitRenegadeChance = 30  # 30%
				if pWinner.isHasPromotion(iPromoLeader):
						iUnitRenegadeChance += 10
				if pWinner.isHasPromotion(gc.getInfoTypeForString("PROMOTION_HERO")):
						iUnitRenegadeChance += 10
...
def doNavalOnCombatResult(pWinner, pLoser, bWinnerIsDead):
		...
		if pLoser.isAnimal(): return bUnitDone
...
def huntingResult(pLoser, pWinner):
		...
		if team.isHasTech(iJagd):
				...
				if pHunterCityPlot is not None:
...
def huntingCity(pUnit):
		...
		iY = pUnit.getY()
		iUnitOwner = pUnit.getOwner()
		# PAE VII: auch bei Vasallen oder MP Team Games
		...
		for i in range(iRange):
			pTmp = gc.getPlayer(i)
			iTmp = pTmp.getTeam()
			if iTmp == iTeam or gc.getTeam(iTmp).isVassal(iTeam):
		...
		# Winner gets Slave
				...
				if pLoser.getUnitType() not in L.LUnitNoSlaves and pLoser.getUnitCombatType() != gc.getInfoTypeForString("UNITCOMBAT_SIEGE"):
						...
						if team.isHasTech(iTechEnslavement):
								# Create a slave unit
								# PAE 7.16e: stay on the same plot, if alone
								if pLoser.plot().getNumUnits() == 1:
									pPlotS = pLoser.plot()
								else:
									pPlotS = pWinner.plot()
								NewUnit = pWinnerPlayer.initUnit(gc.getInfoTypeForString("UNIT_SLAVE"), pPlotS.getX(), pPlotS.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
								NewUnit.finishMoves()
								if pWinnerPlayer.isHuman():
...
def doSettleStatthalter(pUnit, pCity):
		...
		if pUnit.getUnitClassType() == gc.getInfoTypeForString("UNITCLASS_STATTHALTER"):
				iTyp = CvUtil.getScriptData(pUnit, ["typ"], -1)
				# Test Message
...
def getSupplyFromPlot(iPlayer, pPlot):
		...
		# Fremdes Terrain
				...
				if iLoopOwner != -1:
						...
						# feindliches Terrain
						elif gc.getTeam(iTeam).isAtWar(iTeamPlot) and eImprovement != -1:
								# Plot wird beschlagnahmt
								iSupplyChange += 20

		# Allgemeines
		if eImprovement in lImpFood1:
				iSupplyChange += 20
		if eImprovement in lImpFood2:
				iSupplyChange += 15
		# Oase
...
def setUnitCulture(pUnit):
	pPlot = pUnit.plot()
	# iCulture = city.findHighestCulture() Klappt nicht, da Kultur des Stadtplots benoetigt wird
	# Muss selbst bestimmt werden, da plot.findHighestCultureTeam() das Team, aber nicht die Civ zurueckgibt
	iCulture = -1
	iTmp = -1
	iRange = gc.getMAX_PLAYERS()
	for i in range(iRange):
		if pPlot.getCulture(i) > iTmp:
			iTmp = pPlot.getCulture(i)
			iCulture = i

	# Sollte auf dem Plot keine Kultur sein, gehoert der Auswanderer zur Kultur des Besitzers
	if iTmp == -1 or iCulture == -1:
		iCulture = pUnit.getOwner()

	CvUtil.addScriptData(pUnit, "p", iCulture)
	return

# Einheit Religion geben
def setUnitReligion(pUnit):
	pPlot = pUnit.plot()
	if not pPlot.isCity():
		return
	pCity = pPlot.getPlotCity()
...
# Names for Great Generals
def getGGName(pPlayer):
		...
		if loopPlayer is not None and not loopPlayer.isNone() and loopPlayer.isAlive():
			...
			for loopUnit in lUnits:
					if loopUnit is not None and not loopUnit.isNone():
							if loopUnit.isMilitaryHappiness() and loopUnit.getName() not in lUnitNames:
								lUnitNames.append(loopUnit.getName())
	...
	if pPlayer.getCurrentEra() >= 2:
		...
		# 1st: CIV names Iron Age
		if len(listNames):
			for iName in listNames:
				if iName not in GG_UsedNames and iName not in lUnitNames:
					GG_Name = iName
					break

		# 2nd: generic names Iron Age
		if GG_Name == "":
	...
	# save official names only
	if GG_Name != "" and GG_Name not in GG_UsedNames:
		GG_UsedNames.append(GG_Name)

	# 3rd: random names stone/bronze age
	if GG_Name == "":
		listNames = L.LGGStoneAge
		listlength = len(listNames)
		for i in xrange(listlength):
				iRand = CvUtil.myRandom(listlength, "GGNames stone age")
				if listNames[iRand] not in lUnitNames:
...
# Pirate Pillage Village -----------------------------------
def getPillageVillage(pUnit, bPillage):
	iX = pUnit.getX()
	iY = pUnit.getY()
	iOwner = pUnit.getOwner()
	# N, O, S, W only
	...
	for (dx, dy) in directions:
		...
		if loopPlot.isNone() or loopPlot.getOwner() == iOwner:
			continue

		iImp = loopPlot.getImprovementType()
		if iImp > -1 and iImp in L.DCottages:
			if loopPlot.getNumUnits() == 0:
				rank = L.DCottages[iImp]
				if rank > best_imp:
					best_imp = rank
					best_plot = loopPlot

	if bPillage and best_plot != None:
		iImp = best_plot.getImprovementType()
		best_plot.setImprovementType(L.DCottageDowngrade[iImp])
...
def onModNetMessage(argsList):
	...
	if iData1 == 726:
		...
		LPromos = [
			gc.getInfoTypeForString("PROMOTION_PIRAT1"),
			gc.getInfoTypeForString("PROMOTION_PIRAT2"),
			gc.getInfoTypeForString("PROMOTION_PIRAT3")
		]
		...
		if iData2 == 0:
			...
			popupInfo = CyPopupInfo()
			popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON)
			iRand = CvUtil.myRandom(20, "726_header") + 1
			szText = CyTranslator().getText("TXT_KEY_MESSAGE_PIRACY_A" + str(iRand), ())
			if not pUnit.isHasPromotion(LPromos[1]):
				szText = szText + u"\n<color=128,128,128>" + CyTranslator().getText("TXT_KEY_MESSAGE_PIRACY_F1", ()) + u"</color>"
			elif not pUnit.isHasPromotion(LPromos[2]):
				szText = szText + u"\n<color=128,128,128>" + CyTranslator().getText("TXT_KEY_MESSAGE_PIRACY_F2", ()) + u"</color>"
			popupInfo.setText(szText)
			popupInfo.setData1(iData5)  # UnitID
			popupInfo.setData2(iData4)  # iPlayer
			popupInfo.setOnClickedPythonCallback("popupPiratePillageVillage")

			# Button 0: Pillage
			iPromo = LPromos[0]
			popupInfo.addPythonButton(CyTranslator().getText("TXT_KEY_MESSAGE_PIRACY_B1", ()), gc.getPromotionInfo(iPromo).getButton())

			# Button 1: Massacre
			iPromo = LPromos[1]
			if pUnit.isHasPromotion(iPromo):
				popupInfo.addPythonButton(CyTranslator().getText("TXT_KEY_MESSAGE_PIRACY_C1", ()), gc.getPromotionInfo(iPromo).getButton())

			# Button 2: Enslave
			iPromo = LPromos[2]
			if pUnit.isHasPromotion(iPromo):
				popupInfo.addPythonButton(CyTranslator().getText("TXT_KEY_MESSAGE_PIRACY_D1", ()), gc.getPromotionInfo(iPromo).getButton())

			# Cancel button
			iRand = CvUtil.myRandom(14, "726_cancel") + 1
			popupInfo.addPythonButton(CyTranslator().getText("TXT_KEY_MESSAGE_PIRACY_E"+str(iRand), ()), "Art/Interface/Buttons/Actions/Cancel.dds")
			popupInfo.setFlags(popupInfo.getNumPythonButtons()-1)
		...
		# iData3 == iButtonID
			...
			# Button-Check
			if iData3 == 1 and not pUnit.isHasPromotion(LPromos[1]) or iData3 == 2 and not pUnit.isHasPromotion(LPromos[2]):
				argsList = (726,0,0,iData4,iData5)
				onModNetMessage(argsList)
				return

			# pillage plot
			pLootPlot = getPillageVillage(pUnit,False)
			if pLootPlot != None:
				...
				# minValue = ImprovementType * 2 * (2 + PiratePromotions)
				# maxValue = minValue * 2
				iMinValue = L.DCottages[pLootPlot.getImprovementType()] * 2 * (2 + iPromos)
				iLoot = CyGame().getSorenRandNum(iMinValue * 2 - iMinValue + 1, "getPirateLoot") + iMinValue
				pPlayer.changeGold(iLoot)

				# pillage village
				iDestroyChance = 1
				# Button0 (normal pillage)
				if iData3 == 0:
					# Meldung
					if iData4 == gc.getGame().getActivePlayer():
						CyInterface().addMessage(iData4, True, 5, CyTranslator().getText("TXT_KEY_MESSAGE_PIRACY_B2", (iLoot, )), "AS2D_PILLAGE", 2, "Art/Interface/Buttons/Actions/button_action_merc_elite.dds", ColorTypes(8), pLootPlot.getX(), pLootPlot.getY(), True, True)
					elif gc.getPlayer(pLootPlot.getOwner()).isHuman():
						CyInterface().addMessage(pLootPlot.getOwner(), True, 5, CyTranslator().getText("TXT_KEY_MESSAGE_PIRACY_B3", ("",)), "AS2D_PILLAGE", 2, "Art/Interface/Buttons/Actions/button_action_merc_elite.dds", ColorTypes(7), pLootPlot.getX(), pLootPlot.getY(), True, True)
				...
				# massacre
					...
					# Meldung
					if iData4 == gc.getGame().getActivePlayer():
						CyInterface().addMessage(iData4, True, 5, CyTranslator().getText("TXT_KEY_MESSAGE_PIRACY_C2", (iLoot, )), "AS2D_PILLAGE", 2, "Art/Interface/Buttons/Actions/button_action_merc_elite.dds", ColorTypes(8), pLootPlot.getX(), pLootPlot.getY(), True, True)
					elif gc.getPlayer(pLootPlot.getOwner()).isHuman():
						CyInterface().addMessage(pLootPlot.getOwner(), True, 5, CyTranslator().getText("TXT_KEY_MESSAGE_PIRACY_C3", ("",)), "AS2D_PILLAGE", 2, "Art/Interface/Buttons/Actions/button_action_merc_elite.dds", ColorTypes(7), pLootPlot.getX(), pLootPlot.getY(), True, True)

				# enslave
				elif iData3 == 2:
					iDestroyChance = 10
					pPlotSlave = pUnit.plot()
					iSlaveOwner = iData5
					if pUnit.cargoSpace() <= pUnit.getCargo():
						pPlotSlave = pLootPlot
						iSlaveOwner = gc.getBARBARIAN_PLAYER()
					CvUtil.spawnUnit(gc.getInfoTypeForString("UNIT_SLAVE"), pPlotSlave, gc.getPlayer(iSlaveOwner))
					# Meldung
					if iData4 == gc.getGame().getActivePlayer():
						CyInterface().addMessage(iData4, True, 5, CyTranslator().getText("TXT_KEY_MESSAGE_PIRACY_D2", (iLoot, )), "AS2D_PILLAGE", 2, "Art/Interface/Buttons/Actions/button_action_merc_elite.dds", ColorTypes(8), pLootPlot.getX(), pLootPlot.getY(), True, True)
					elif gc.getPlayer(pLootPlot.getOwner()).isHuman():
						CyInterface().addMessage(pLootPlot.getOwner(), True, 5, CyTranslator().getText("TXT_KEY_MESSAGE_PIRACY_D3", ("",)), "AS2D_PILLAGE", 2, "Art/Interface/Buttons/Actions/button_action_merc_elite.dds", ColorTypes(7), pLootPlot.getX(), pLootPlot.getY(), True, True)


				# destroy improvement
				if iDestroyChance > CvUtil.myRandom(100, "726_DestroyVillage"):
					getPillageVillage(pUnit,True)
					if iData4 == gc.getGame().getActivePlayer():
						CyAudioGame().Play2DSound("AS2D_DESTROY")

					# village people leave the plot
					if 2 > CvUtil.myRandom(10, "726_WillVillagersMigrate"):
						# seek city
						# Umkreis von 3 Feldern
						iRange = 3
						iX = pLootPlot.getX()
						iY = pLootPlot.getY()
						LCities = []
						for i in range(-iRange, iRange+1):
							for j in range(-iRange, iRange+1):
								...
								if loopPlot.isCity() and loopPlot.getOwner() == pLootPlot.getOwner():
									LCities.append(loopPlot.getPlotCity())

						if len(LCities):
							iRand = CvUtil.myRandom(len(LCities), "726_VillagersMigrate")
							LCities[iRand].changePopulation(1)
							if gc.getPlayer(pLootPlot.getOwner()).isHuman():
								CyInterface().addMessage(pLootPlot.getOwner(), True, 5, CyTranslator().getText("TXT_KEY_MESSAGE_PIRACY_B4", (LCities[iRand].getName(),)),
													"AS2D_PLAGUE", 2, "Art/Interface/Buttons/Actions/button_action_merc_elite.dds", ColorTypes(13), LCities[iRand].getX(), LCities[iRand].getY(), True, True)
							# check city status
							PAE_City.doCheckCityState(LCities[iRand])


				# Unit gets Promo
				iChancePromo = 2
				if iChancePromo > CvUtil.myRandom(10, "726_promo"):
					iNewPromo = -1
					for iPromo in LPromos:
						if not pUnit.isHasPromotion(iPromo):
							iNewPromo = iPromo
					...
					if iNewPromo != -1:
						pUnit.setHasPromotion(iNewPromo, True)
						if iData4 == gc.getGame().getActivePlayer():
								CyInterface().addMessage(iData4, True, 5, CyTranslator().getText("TXT_KEY_EVENT_UNIT_PROMOTION", (pUnit.getName(),gc.getPromotionInfo(iNewPromo).getDescription())),
													"AS2D_IF_LEVELUP", 2, gc.getPromotionInfo(iNewPromo).getButton(), ColorTypes(8), pUnit.getX(), pUnit.getY(), True, True)
	...
	# Schiff reparieren / Crew austauschen
		...
		# Crew austauschen
			...
			pPlayer = gc.getPlayer(iData4)
			pUnit = pPlayer.getUnit(iData5)
			#iCost = pUnit.baseCombatStr() * 5 + pUnit.baseMoves() * 3
			iCost = 20
			# confirm
			if iData3 == 1:
				for iPromo in L.LCombatPromos:
					if pUnit.isHasPromotion(iPromo):
						pUnit.setHasPromotion(iPromo, False)
				#pUnit.setLevel(0)
				pPlayer.changeGold(-iCost)
			...
			# PopUp
				...
				popupInfo = CyPopupInfo()
				popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON)
				popupInfo.setText(CyTranslator().getText("TXT_KEY_MESSAGE_UNSET_PROMO_ANGST_SEA", (iCost, )))
				popupInfo.setOnClickedPythonCallback("popupSellCrew")
				popupInfo.setData1(iData4)
				popupInfo.setData2(iData5)
				popupInfo.addPythonButton(CyTranslator().getText("TXT_KEY_POPUP_YES1", ()), "")
