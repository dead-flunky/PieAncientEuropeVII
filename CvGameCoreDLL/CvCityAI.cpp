...
void CvCityAI::AI_assignWorkingPlots()
{
	...
	if ((getOwnerINLINE() == GC.getGameINLINE().getActivePlayerInternal()) && isCitySelected())
	{
...
void CvCityAI::AI_chooseProduction()
{
	...
	if (isBarbarian())
	{
		...
		if (!AI_isDefended(plot()->plotCount(PUF_isUnitAIType, UNITAI_ATTACK, -1, getOwnerINLINE())))
		{
...
UnitTypes CvCityAI::AI_bestUnit(bool bAsync, AdvisorTypes eIgnoreAdvisor, UnitAITypes* peBestUnitAI)
{
	...
	if (bWarPossible)
	{
		...
		{
			// PBMod
			aiUnitAIVal[UNITAI_ICBM] += std::max((GET_PLAYER(getOwnerINLINE()).getTotalPopulation() / 25), ((GC.getGameINLINE().countCivPlayersAlive(true) + GC.getGameINLINE().countTotalNukeUnits()) / (GC.getGameINLINE().countCivPlayersAlive(true) + 1)));
		}
	}
...
bool CvCityAI::AI_chooseUnit(UnitAITypes eUnitAI)
{
	...
	if (eBestUnit != NO_UNIT)
	{
		...
		gDLL->getInterfaceIFace()->addMessage(
			GC.getGameINLINE().getActivePlayerInternal(), false, GC.getEVENT_MESSAGE_TIME(),
			CvWString::format(L"XXX"),"", MESSAGE_TYPE_INFO
		);
		// PAE Test
		gDLL->getInterfaceIFace()->addMessage(
			GC.getGameINLINE().getActivePlayerInternal(), false, GC.getEVENT_MESSAGE_TIME(),
			CvWString::format(L"Stadt %s waehlt Einheit: %s (%d)",
            getName().GetCString(),
