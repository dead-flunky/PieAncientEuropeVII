...
{
	if (ePlayer != GC.getGameINLINE().getActivePlayerInternal())
	{
		if (GET_PLAYER(ePlayer).isHuman())
		{
...
void CvGameTextMgr::setMinimizePopupHelp(CvWString& szString, const CvPopupInfo & info)
{
	...
	case BUTTONPOPUP_CHOOSEPRODUCTION:
		pCity = GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).getCity(info.getData1());
		if (pCity != NULL)
		{
			eTrainUnit = NO_UNIT;
...
void CvGameTextMgr::setUnitHelp(CvWStringBuffer &szString, const CvUnit* pUnit, bool bOneLine, bool bShort)
{
	...
	if (pUnit->getOwnerINLINE() != GC.getGameINLINE().getActivePlayerInternal() && !pUnit->isAnimal() && !pUnit->getUnitInfo().isHiddenNationality())
	{
		szString.append(L", ");
			...
			for (int iI = 0; iI < GC.getNumUnitInfos(); ++iI)
			{
				for (int iJ = 0; iJ < MAX_PLAYERS; iJ++)
				{
					...
					if (aiUnitNumbers[iIndex] > 0)
					{
						...
						if (iJ != GC.getGameINLINE().getActivePlayerInternal() && !GC.getUnitInfo((UnitTypes)iI).isAnimal() && !GC.getUnitInfo((UnitTypes)iI).isHiddenNationality())
						{
							szString.append(L", ");
	...
	*/
	...
	if (pAttacker != NULL)
	{
		...
		if (pDefender != NULL && pDefender != pAttacker && pDefender->canDefend(pPlot) && pAttacker->canAttack(*pDefender))
		{
			...
			// PAE: show current HP of the fighting units (06.06.2026)
			float hpA = float(pAttacker->baseCombatStr()) / 100 * pAttacker->currHitPoints();
			float hpB = float(pDefender->baseCombatStr()) / 100 * pDefender->currHitPoints();
			szString.append(CvWString::format(L" (%s%.1f%s%c %s %s%.1f%s%c)",
					CvWString::format(SETCOLR, TEXT_COLOR("COLOR_POSITIVE_TEXT")).c_str(),
					hpA,
					CvWString::format(ENDCOLR).c_str(),
					gDLL->getSymbolID(STRENGTH_CHAR),
					gDLL->getText("TXT_KEY_COMBAT_MESSAGE_VS").c_str(),
					CvWString::format(SETCOLR, TEXT_COLOR("COLOR_NEGATIVE_TEXT")).c_str(),
					hpB,
					CvWString::format(ENDCOLR).c_str(),
					gDLL->getSymbolID(STRENGTH_CHAR)
				)
			...
			// PAE new combat system: battle rounds / Neues Kampfsystem 05.06.2026 (Pie)
			if (!GC.getGameINLINE().isOption(GAMEOPTION_NO_PAE_COMBAT) && pAttacker->getDomainType() != DOMAIN_AIR)
			{
				// PAE Combat Rounds
				int iRounds = CvUnit::getCombatRounds(pAttacker, pDefender, pPlot);
				szString.append(NEWLINE);
				...
				/** Copied from BULL mod: Many thanks to DanF5771 for some of these calculations! **/
				...
				float AttackerBaseHP = float(pAttacker->baseCombatStr());
				float Damage2Attacker = float(pAttacker->baseCombatStr()) / 100 * iDamageToAttacker;
				float Damage2Defender = float(pDefender->baseCombatStr()) / 100 * iDamageToDefender;

				// Damage per round
				szString.append(NEWLINE);
				szString.append(gDLL->getText("TXT_KEY_PAE_COMBAT_ROUNDS_INFO_DAMAGE_PER_ROUND"));
				szString.append(NEWLINE);
				szString.append(gDLL->getText("TXT_KEY_PAE_COMBAT_ROUNDS_INFO_YOU"));
				szString.append(CvWString::format(L": %s%.1f%s%c (%d %s), ",
						CvWString::format(SETCOLR, TEXT_COLOR("COLOR_POSITIVE_TEXT")).c_str(),
						Damage2Defender, CvWString::format(ENDCOLR).c_str(),
						gDLL->getSymbolID(STRENGTH_CHAR), iDamageToDefender, gDLL->getText("TXT_KEY_PAE_COMBAT_ROUNDS_INFO_TP").c_str()
					)
				);
				szString.append(gDLL->getText("TXT_KEY_PAE_COMBAT_ROUNDS_INFO_ENEMY"));
				szString.append(CvWString::format(L": %s%.1f%s%c (%d %s)",
						CvWString::format(SETCOLR, TEXT_COLOR("COLOR_NEGATIVE_TEXT")).c_str(),
						Damage2Attacker, CvWString::format(ENDCOLR).c_str(),
						gDLL->getSymbolID(STRENGTH_CHAR), iDamageToAttacker, gDLL->getText("TXT_KEY_PAE_COMBAT_ROUNDS_INFO_TP").c_str()
					)
				);

				// First Strikes
				int iFirstStrikesPlayer = 0;
				int iFirstStrikesGegner = 0;
				bool bFirstStrike = false;
				szString.append(NEWLINE);
				szString.append(gDLL->getText("TXT_KEY_PAE_COMBAT_ROUNDS_INFO_FIRST_STRIKES"));
				if (pAttacker->immuneToFirstStrikes() && pDefender->immuneToFirstStrikes()) szString.append(NEWLINE);
				else szString.append(L" ");

				// PLAYER / SPIELER
				szString.append(gDLL->getText("TXT_KEY_PAE_COMBAT_ROUNDS_INFO_YOU"));
				if (pAttacker->immuneToFirstStrikes())
					szString.append(CvWString::format(L" %s(%s)%s", CvWString::format(SETCOLR, TEXT_COLOR("COLOR_POSITIVE_TEXT")).c_str(), gDLL->getText("TXT_KEY_PAE_COMBAT_ROUNDS_INFO_IMMUN").c_str(), CvWString::format(ENDCOLR).c_str()));
				szString.append(L": ");

				// Player Values (set color)
				// Spieler und Gegner sind immun
				if (pAttacker->immuneToFirstStrikes() && pDefender->immuneToFirstStrikes()) {
					szString.append(CvWString::format(SETCOLR, TEXT_COLOR("COLOR_GREY")));
				// Spieler ist immun, Gegner nicht
				} else if (pAttacker->immuneToFirstStrikes() && !(pDefender->immuneToFirstStrikes())) {
				...
				// Spieler ist nicht immun, Gegner schon
				} else if (pDefender->immuneToFirstStrikes()) {
					szString.append(CvWString::format(SETCOLR, TEXT_COLOR("COLOR_GREY")));
				// Keiner ist immun
				} else {
					if (pAttacker->maxFirstStrikes() == 0 && pDefender->maxFirstStrikes() == 0)
						szString.append(CvWString::format(SETCOLR, TEXT_COLOR("COLOR_GREY")));
					else if (pAttacker->maxFirstStrikes() >= pDefender->maxFirstStrikes())
						szString.append(CvWString::format(SETCOLR, TEXT_COLOR("COLOR_POSITIVE_TEXT")));
					else
				...
				// Player Values (show values) Player FIX / CHANCE
				if (pAttacker->firstStrikes() == pAttacker->maxFirstStrikes()) {
					szString.append(CvWString::format(L"%d", pAttacker->firstStrikes()));
					if (bFirstStrike) iFirstStrikesPlayer = pAttacker->firstStrikes();
				} else {
					szString.append(CvWString::format(L"%d-%d", pAttacker->firstStrikes(), pAttacker->maxFirstStrikes()));
					if (bFirstStrike) iFirstStrikesPlayer = pAttacker->maxFirstStrikes();
				}

				// Player Values (end color)
				szString.append(CvWString::format(ENDCOLR) + L", ");
				//szString.append(CvWString::format(L"%s, ", CvWString::format(ENDCOLR).c_str()));
				...
				if (pDefender->immuneToFirstStrikes())
					szString.append(CvWString::format(L" %s(%s)%s", CvWString::format(SETCOLR, TEXT_COLOR("COLOR_NEGATIVE_TEXT")).c_str(), gDLL->getText("TXT_KEY_PAE_COMBAT_ROUNDS_INFO_IMMUN").c_str(), CvWString::format(ENDCOLR).c_str()));
				szString.append(L": ");

				// Opponent Values (set color)
				// Spieler und Gegner sind immun
				if (pAttacker->immuneToFirstStrikes() && pDefender->immuneToFirstStrikes()) {
					szString.append(CvWString::format(SETCOLR, TEXT_COLOR("COLOR_GREY")));
				// Spieler ist immun, Gegner nicht
				...
				// Opponent Values (show values) Player FIX / CHANCE
				if (pDefender->firstStrikes() == pDefender->maxFirstStrikes()) {
					szString.append(CvWString::format(L"%d", pDefender->firstStrikes()));
					if (bFirstStrike) iFirstStrikesGegner = pDefender->firstStrikes();
				} else {
					szString.append(CvWString::format(L"%d-%d", pDefender->firstStrikes(), pDefender->maxFirstStrikes()));
					if (bFirstStrike) iFirstStrikesGegner = pDefender->maxFirstStrikes();
				}

				// Opponent Values (end color)
				...
				// Attack chance
				float ChanceOfAttack = float(iAttackerOdds)*100.0f / float(GC.getDefineINT("COMBAT_DIE_SIDES"));
				szString.append(NEWLINE);
				szString.append(gDLL->getText("TXT_KEY_PAE_COMBAT_ROUNDS_INFO_HIT_CHANCE"));
				if (ChanceOfAttack < 30) szString.append(CvWString::format(SETCOLR, TEXT_COLOR("COLOR_NEGATIVE_TEXT")));
				else if (ChanceOfAttack < 80) szString.append(CvWString::format(SETCOLR, TEXT_COLOR("COLOR_PLAYER_ORANGE_TEXT")));
				else szString.append(CvWString::format(SETCOLR, TEXT_COLOR("COLOR_POSITIVE_TEXT")));
				szString.append(CvWString::format(L" %.1f%%", ChanceOfAttack));
				szString.append(CvWString::format(ENDCOLR));

				// Winning results
				szString.append(NEWLINE);
				szString.append(gDLL->getText("TXT_KEY_PAE_COMBAT_ROUNDS_INFO_VICTORIOUS_ROUNDS"));

				int i=0;
				while (i <= iRounds) {
					szString.append(NEWLINE);
					szString.append(CvWString::format(L"%d ", iRounds-i));
					if (iRounds-i == 1) szString.append(gDLL->getText("TXT_KEY_PAE_COMBAT_ROUNDS_INFO_ROUND"));
					else szString.append(gDLL->getText("TXT_KEY_PAE_COMBAT_ROUNDS_INFO_ROUNDS"));
					szString.append(L": ");
					// Colors
					if (hpA - Damage2Attacker * i > AttackerBaseHP / 2)
						szString.append(CvWString::format(SETCOLR, TEXT_COLOR("COLOR_POSITIVE_TEXT")));
					else if (hpA - Damage2Attacker * i <= 0)
						szString.append(CvWString::format(SETCOLR, TEXT_COLOR("COLOR_NEGATIVE_TEXT")));
					else
						szString.append(CvWString::format(SETCOLR, TEXT_COLOR("COLOR_PLAYER_ORANGE_TEXT")));
					// Value
					if (hpA - Damage2Attacker * i <= 0)
						szString.append(L"0");
					else
						szString.append(CvWString::format(L"%.1f", hpA - Damage2Attacker * i));
					szString.append(CvWString::format(ENDCOLR));
					szString.append(CvWString::format(L"%c", gDLL->getSymbolID(STRENGTH_CHAR)));
					i++;
				}
				...
				// Survival chance
				float pSurvivalChance = 0.0f;
				float pB = 100.0f - ChanceOfAttack;

				// reduce possible First Strikes
				hpA -= iFirstStrikesGegner * Damage2Attacker;
				hpB -= iFirstStrikesPlayer * Damage2Defender;

				// Hits to kill Attacker/Defender
				int hitsToKillA = (int)ceil(hpA / Damage2Attacker);
				int hitsToKillB = (int)ceil(hpB / Damage2Defender);

				if (hpA <= 0) {}
				else if (hpB <= 0 || hitsToKillA > iRounds) pSurvivalChance = 100.0f;
				else {

					// --- Binomialkoeffizient C(n,k) ---
					// P(K = k) für k = 0..hitsToKillA
					// P(K = k) = C(iRounds, k) * pB^k * ChanceOfAttack^(iRounds-k)
					int c;
					for (int k = 0; k <= hitsToKillA - 1; ++k)
					{
						if (k < 0 || k > iRounds) c = 0;
						else if (k == 0 || k == iRounds) c = 1;
						else {
							c = 1;
							for (int i = 1; i <= k; ++i) c = c * (iRounds - (i - 1)) / i;
						}
						pSurvivalChance += c * pow(pB/100.0f, k) * pow(ChanceOfAttack/100.0f, iRounds - k);
					}
					pSurvivalChance *= 100.0f;
				}

				szString.append(NEWLINE);
				szString.append(gDLL->getText("TXT_KEY_PAE_COMBAT_ROUNDS_SURVIVE"));
				if (pSurvivalChance < 50.0) szString.append(gDLL->getText("TXT_KEY_COLOR_NEGATIVE"));
				else if (pSurvivalChance < 80.0) szString.append(CvWString::format(SETCOLR, TEXT_COLOR("COLOR_PLAYER_ORANGE_TEXT")));
				else szString.append(gDLL->getText("TXT_KEY_COLOR_POSITIVE"));
				szString.append(CvWString::format(L" %.2f%%", pSurvivalChance));
				szString.append(CvWString::format(ENDCOLR));

				// min hit rounds to kill enemy
				szString.append(NEWLINE);
				szString.append(gDLL->getText("TXT_KEY_PAE_COMBAT_ROUNDS_INFO_ENEMY_DEATH"));
				if (hitsToKillB <= iRounds) {
					szString.append(gDLL->getText("TXT_KEY_COLOR_POSITIVE"));
					szString.append(CvWString::format(L" %d ", hitsToKillB + 1));
					szString.append(CvWString::format(ENDCOLR));
				} else {
					szString.append(CvWString::format(L" %d ", hitsToKillB + 1));
				}
				if (hitsToKillB + 1 == 1) szString.append(gDLL->getText("TXT_KEY_PAE_COMBAT_ROUNDS_INFO_ROUND"));
				else szString.append(gDLL->getText("TXT_KEY_PAE_COMBAT_ROUNDS_INFO_ROUNDS"));

			}
			// -- PAE new combat system end ----------------------
			...
			if ((gDLL->getChtLvl() > 0))
			{
				...
				int iOurStrengthDefense = GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).AI_getOurPlotStrength(pPlot, 1, true, false);
				int iOurStrengthOffense = GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).AI_getOurPlotStrength(pPlot, 1, false, false);
				szTempBuffer.Format(L"\nPlot Strength(Ours)= d%d, o%d", iOurStrengthDefense, iOurStrengthOffense);
				szString.append(szTempBuffer);
				int iEnemyStrengthDefense = GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).AI_getEnemyPlotStrength(pPlot, 1, true, false);
				int iEnemyStrengthOffense = GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).AI_getEnemyPlotStrength(pPlot, 1, false, false);
...
//			}
	...
	else if (bShift && !bAlt && (gDLL->getChtLvl() > 0))
	{
		...
		for (iI = 0; iI < GC.getNumBonusInfos(); ++iI)
		{
			if (pPlot->isPlotGroupConnectedBonus(GC.getGameINLINE().getActivePlayerInternal(), ((BonusTypes)iI)))
			{
				szString.append(NEWLINE);
				szString.append(GC.getBonusInfo((BonusTypes)iI).getDescription());
				szString.append(CvWString::format(L" (%d)", GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).AI_bonusVal((BonusTypes)iI)));
			}
		}

		if (pPlot->getPlotGroup(GC.getGameINLINE().getActivePlayerInternal()) != NULL)
		{
			szTempBuffer.Format(L"\n(%d, %d) group: %d", pPlot->getX_INLINE(), pPlot->getY_INLINE(), pPlot->getPlotGroup(GC.getGameINLINE().getActivePlayerInternal())->getID());
		}
		else
		{
		...
		PlayerTypes eActivePlayer = GC.getGameINLINE().getActivePlayerInternal();
	...
	if (pPlot->getNumUnits() > 1) {
		szString.append(NEWLINE);
		szString.append(gDLL->getText("TXT_KEY_PEDIA_CATEGORY_UNIT"));
		szString.append(CvWString::format(L": %d", pPlot->getNumUnits()));
	}
...
void CvGameTextMgr::parseLeaderTraits(CvWStringBuffer &szHelpString, PlayerTypes ePlayer)
{
	...
	if (eLeader != NO_LEADER)
	{
		CivilizationTypes eCiv = GET_PLAYER(ePlayer).getCivilizationType();
		...
		for (iI = 0; iI < GC.getNumTraitInfos(); iI++)
		{
			if (GC.getLeaderHeadInfo(eLeader).hasTrait(iI) || GET_PLAYER(ePlayer).hasTrait((TraitTypes)iI))
			{
				parseTraits(szHelpString, ((TraitTypes)iI), eCiv, false);
			}
		}
	}
	else
...
void CvGameTextMgr::parseSpecialistHelp(CvWStringBuffer &szHelpString, SpecialistTypes eSpecialist, CvCity* pCity, bool bCivilopediaText)
{
	...
	if (eSpecialist != NO_SPECIALIST)
	{
		...
		for (iI = 0; iI < NUM_YIELD_TYPES; ++iI)
		{
			...
			{
				aiYields[iI] = GET_PLAYER((pCity != NULL) ? pCity->getOwnerINLINE() : GC.getGameINLINE().getActivePlayerInternal()).specialistYield(eSpecialist, ((YieldTypes)iI));
			}
		}
		...
		for (iI = 0; iI < NUM_COMMERCE_TYPES; ++iI)
		{
			...
			{
				aiCommerces[iI] = GET_PLAYER((pCity != NULL) ? pCity->getOwnerINLINE() : GC.getGameINLINE().getActivePlayerInternal()).specialistCommerce(((SpecialistTypes)eSpecialist), ((CommerceTypes)iI));
			}
		}
...
void CvGameTextMgr::parseCivicInfo(CvWStringBuffer &szHelpText, CivicTypes eCivic, bool bCivilopediaText, bool bPlayerContext, bool bSkipName)
{
	...
	FAssert(GC.getGameINLINE().getActivePlayerInternal() != NO_PLAYER || !bPlayerContext);
	...
	if (!bCivilopediaText)
	{
		if (!bPlayerContext || !(GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).canDoCivics(eCivic)))
		{
	...
	{
		if (bPlayerContext && (GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).getStateReligion() != NO_RELIGION))
		{
			szHelpText.append(NEWLINE);
			szHelpText.append(gDLL->getText("TXT_KEY_CIVIC_GREAT_PEOPLE_MOD_RELIGION", GC.getCivicInfo(eCivic).getStateReligionGreatPeopleRateModifier(), GC.getReligionInfo(GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).getStateReligion()).getChar()));
		}
		else
		{
			szHelpText.append(NEWLINE);
	...
	if (GC.getCivicInfo(eCivic).getStateReligionHappiness() != 0)
	{
		...
		{
			szHelpText.append(NEWLINE);
			szHelpText.append(gDLL->getText("TXT_KEY_CIVIC_STATE_RELIGION_HAPPINESS", abs(GC.getCivicInfo(eCivic).getStateReligionHappiness()), ((GC.getCivicInfo(eCivic).getStateReligionHappiness() > 0) ? gDLL->getSymbolID(HAPPY_CHAR) : gDLL->getSymbolID(UNHAPPY_CHAR)), GC.getReligionInfo(GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).getStateReligion()).getChar()));
		}
		else
		{
			szHelpText.append(NEWLINE);
	...
	if (GC.getCivicInfo(eCivic).getStateReligionUnitProductionModifier() != 0)
	{
		...
		{
			szHelpText.append(NEWLINE);
			szHelpText.append(gDLL->getText("TXT_KEY_CIVIC_REL_TRAIN_BONUS", GC.getReligionInfo(GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).getStateReligion()).getChar(), GC.getCivicInfo(eCivic).getStateReligionUnitProductionModifier()));
		}
		else
		{
			szHelpText.append(NEWLINE);
	...
	if (GC.getCivicInfo(eCivic).getStateReligionBuildingProductionModifier() != 0)
	{
		...
		{
			szHelpText.append(NEWLINE);
			szHelpText.append(gDLL->getText("TXT_KEY_CIVIC_REL_BUILDING_BONUS", GC.getReligionInfo(GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).getStateReligion()).getChar(), GC.getCivicInfo(eCivic).getStateReligionBuildingProductionModifier()));
		}
		else
		{
			szHelpText.append(NEWLINE);
	...
	if (GC.getCivicInfo(eCivic).getStateReligionFreeExperience() != 0)
	{
		...
		{
			szHelpText.append(NEWLINE);
			szHelpText.append(gDLL->getText("TXT_KEY_CIVIC_REL_FREE_XP", GC.getCivicInfo(eCivic).getStateReligionFreeExperience(), GC.getReligionInfo(GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).getStateReligion()).getChar()));
		}
		else
		{
			szHelpText.append(NEWLINE);
	...
	for (iI = 0; iI < GC.getNumBuildingClassInfos(); ++iI)
	{
		...
		{
			if (bPlayerContext && NO_PLAYER != GC.getGameINLINE().getActivePlayerInternal())
			{
...
void CvGameTextMgr::setTechHelp(CvWStringBuffer &szBuffer, TechTypes eTech, bool bCivilopediaText, bool bPlayerContext, bool bStrategyText, bool bTreeInfo, TechTypes eFromTech)
{
	...
	for (iI = 0; iI < GC.getNumBuildingClassInfos(); ++iI)
	{
		if (!bPlayerContext || (GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).getBuildingClassCount((BuildingClassTypes)iI) > 0))
		{
			if (GC.getGameINLINE().getActivePlayerInternal() != NO_PLAYER)
			{
	...
	if (!bCivilopediaText)
	{
		...
		for (iI = 0; iI < GC.getNumUnitClassInfos(); ++iI)
		{
			if (!bPlayerContext || !(GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).isProductionMaxedUnitClass((UnitClassTypes)iI)))
			{
				if (GC.getGameINLINE().getActivePlayerInternal() != NO_PLAYER)
				{
				...
				{
					if (!bPlayerContext || !(GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).canTrain(eLoopUnit)))
					{
		...
		for (iI = 0; iI < GC.getNumBuildingClassInfos(); ++iI)
		{
			if (!bPlayerContext || !(GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).isProductionMaxedBuildingClass((BuildingClassTypes)iI)))
			{
				if (GC.getGameINLINE().getActivePlayerInternal() != NO_PLAYER)
				{
					eLoopBuilding = (BuildingTypes)GC.getCivilizationInfo(GC.getGameINLINE().getActiveCivilizationType()).getCivilizationBuildings(iI);
				...
				if (eLoopBuilding != NO_BUILDING)
				{
					if (!bPlayerContext || !(GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).canConstruct(eLoopBuilding, false, true)))
					{
		...
		{
			if (!bPlayerContext || !(GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).isProductionMaxedProject((ProjectTypes)iI)))
			{
				if (!bPlayerContext || !(GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).canCreate(((ProjectTypes)iI), false, true)))
				{
	...
	if (!bCivilopediaText)
	{
		...
		{
			szBuffer.append(NEWLINE);
			szBuffer.append(gDLL->getText("TXT_KEY_TECH_NUM_TURNS", GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).getResearchTurnsLeft(eTech, (gDLL->ctrlKey() || !(gDLL->shiftKey())))));
	...
	if (GC.getGameINLINE().getActivePlayerInternal() != NO_PLAYER)
	{
		if (GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).canResearch(eTech))
		{
			for (iI = 0; iI < GC.getNumUnitInfos(); ++iI)
			{
				...
				{
					if (::getDiscoveryTech((UnitTypes)iI, GC.getGameINLINE().getActivePlayerInternal()) == eTech)
					{
						szBuffer.append(NEWLINE);
			...
			if (GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).getCurrentEra() < GC.getTechInfo(eTech).getEra())
			{
				szBuffer.append(NEWLINE);
	...
	if (bStrategyText)
	{
		...
		{
			if ((GC.getGameINLINE().getActivePlayerInternal() == NO_PLAYER) || GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).isOption(PLAYEROPTION_ADVISOR_HELP))
			{
...
void CvGameTextMgr::setUnitHelp(CvWStringBuffer &szBuffer, UnitTypes eUnit, bool bCivilopediaText, bool bStrategyText, bool bTechChooserText, CvCity* pCity)
{
	...
	if ((pCity == NULL) || !(pCity->canTrain(eUnit)))
	{
		...
		if (!bCivilopediaText)
		{
			...
			if (!bTechChooserText)
			{
				...
				{
					if (GC.getGameINLINE().getActivePlayerInternal() == NO_PLAYER || !(GET_TEAM(GET_PLAYER(ePlayer).getTeam()).isHasTech((TechTypes)(GC.getUnitInfo(eUnit).getPrereqAndTech()))))
					{
						szBuffer.append(NEWLINE);
			...
			for (iI = 0; iI < GC.getNUM_UNIT_AND_TECH_PREREQS(); ++iI)
			{
				...
				{
					if (bTechChooserText || GC.getGameINLINE().getActivePlayerInternal() == NO_PLAYER || !(GET_TEAM(GET_PLAYER(ePlayer).getTeam()).isHasTech((TechTypes)(GC.getUnitInfo(eUnit).getPrereqAndTechs(iI)))))
					{
						szTempBuffer.Format(L"%s%s", NEWLINE, gDLL->getText("TXT_KEY_REQUIRES").c_str());
	...
	if (!bCivilopediaText && GC.getGameINLINE().getActivePlayerInternal() != NO_PLAYER)
	{
		if (pCity == NULL)
		{
...
void CvGameTextMgr::setBuildingHelp(CvWStringBuffer &szBuffer, BuildingTypes eBuilding, bool bCivilopediaText, bool bStrategyText, bool bTechChooserText, CvCity* pCity)
{
	...
	// PAE Teil 1/2 hierher verschoben
...
void CvGameTextMgr::setBonusHelp(CvWStringBuffer &szBuffer, BonusTypes eBonus, bool bCivilopediaText)
{
	...
	{		
		...
		{
			CvPlayer& kActivePlayer = GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal());
...
void CvGameTextMgr::setReligionHelp(CvWStringBuffer &szBuffer, ReligionTypes eReligion, bool bCivilopedia)
{
	...
	if (religion.getFreeUnitClass() != NO_UNITCLASS)
	{
		if (GC.getGameINLINE().getActivePlayerInternal() != NO_PLAYER)
		{
			eFreeUnit = ((UnitTypes)(GC.getCivilizationInfo(GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).getCivilizationType()).getCivilizationUnits(religion.getFreeUnitClass())));
		}
		else
		{
...
void CvGameTextMgr::setCorporationHelp(CvWStringBuffer &szBuffer, CorporationTypes eCorporation, bool bCivilopedia)
{
	...
	if (kCorporation.getFreeUnitClass() != NO_UNITCLASS)
	{
		if (GC.getGameINLINE().getActivePlayerInternal() != NO_PLAYER)
		{
			eFreeUnit = ((UnitTypes)(GC.getCivilizationInfo(GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).getCivilizationType()).getCivilizationUnits(kCorporation.getFreeUnitClass())));
		}
		else
		{
...
void CvGameTextMgr::buildFreeUnitString(CvWStringBuffer &szBuffer, TechTypes eTech, bool bList, bool bPlayerContext)
{
	...
	if (GC.getGameINLINE().getActivePlayerInternal() != NO_PLAYER)
	{
		eFreeUnit = GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).getTechFreeUnit(eTech);
	}
	else
	{
...
void CvGameTextMgr::buildSingleLineTechTreeString(CvWStringBuffer &szBuffer, TechTypes eTech, bool bPlayerContext)
{
	...
	for (int iI = 0; iI < GC.getNumTechInfos(); ++iI)
	{
		...
		if (bPlayerContext)
		{
			bTechAlreadyAccessible = (GET_TEAM(GC.getGameINLINE().getActiveTeam()).isHasTech((TechTypes)iI) || GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).canResearch((TechTypes)iI));
		}
...
void CvGameTextMgr::setEventHelp(CvWStringBuffer& szBuffer, EventTypes eEvent, int iEventTriggeredId, PlayerTypes ePlayer)
{
	...
	for (int i = 0; i < GC.getNumEventInfos(); ++i)
	{
		if (0 == kEvent.getAdditionalEventTime(i))
		{
			...
			{
				if (GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).canDoEvent((EventTypes)i, *pTriggeredData))
				{
					szTemp.clear();
...
{
	CvPlayer& kPlayer = GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal());
	...
	if (kMission.getCityInsertCultureCostFactor() > 0)
	{
		if (NULL != pPlot)
		{
			...
			if (NULL != pCity && pPlot->getCulture(GC.getGameINLINE().getActivePlayerInternal()) > 0)
			{
...
void CvGameTextMgr::getInterfaceCenterText(CvWString& strText)
{
	...
	if (!gDLL->getInterfaceIFace()->isCityScreenUp())
	{
		...
		else if (!(GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).isAlive()))
		{
...
{
	// Unit cost -1 denotes unit which may not be purchased
	int iCost = GET_PLAYER(GC.getGameINLINE().getActivePlayerInternal()).getAdvancedStartVisibilityCost(true);
	if (iCost > 0)
	{
