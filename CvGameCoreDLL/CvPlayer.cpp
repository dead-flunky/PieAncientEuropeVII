...
void CvPlayer::doTurnUnits()
{
	...
	if (getID() == GC.getGameINLINE().getActivePlayerInternal())
	{
...
void CvPlayer::contact(PlayerTypes ePlayer)
{
	...
	else
	{
		...
		gDLL->getInterfaceIFace()->setDiploQueue(pDiplo, GC.getGameINLINE().getActivePlayerInternal());
	}
}
...
bool CvPlayer::canTrain(UnitTypes eUnit, bool bContinue, bool bTestVisible, bool bIgnoreCost) const
{
	...
	// PAE - units can keep their class when changing civ. Cannot train anyways. 
	...
	if (isTurnActive() != bNewValue)
	{
		...
		if (isTurnActive())
		{
			...
			if (bDoTurn)
			{
				...
				if ((getID() == GC.getGameINLINE().getActivePlayerInternal()) && (GC.getGameINLINE().getElapsedGameTurns() > 0))
				{
	...
	if (getCurrentEra() != eNewValue)
	{
		...
		if (GC.getGameINLINE().getActiveTeam() != NO_TEAM)
		{
			for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
			{
				...
				{
					if ((pLoopPlot->getOwnerINLINE() == getID()) || (!(pLoopPlot->isOwned()) && (getID() == GC.getGameINLINE().getActivePlayerInternal())))
					{
...
bool CvPlayer::isBuildingClassMaxedOut(BuildingClassTypes eIndex, int iExtra) const
{
	...
	FAssertMsg((getBuildingClassCount(eIndex) + iExtra) >= (GC.getBuildingClassInfo(eIndex).getMaxPlayerInstances() + GC.getBuildingClassInfo(eIndex).getExtraPlayerInstances()), "BuildingClassCount + extra >=  getMaxPlayerInstances + getExtraPlayerInstances");
...
bool CvPlayer::doEspionageMission(EspionageMissionTypes eMission, PlayerTypes eTargetPlayer, CvPlot* pPlot, int iExtraData, CvUnit* pSpyUnit)
{
	...
	else if (getID() == GC.getGameINLINE().getActivePlayerInternal())
	{
...
void CvPlayer::doAdvancedStartAction(AdvancedStartActionTypes eAction, int iX, int iY, int iData, bool bAdd)
{
	...
	case ADVANCEDSTARTACTION_EXIT:
		...
		if (GC.getGameINLINE().getActivePlayerInternal() == getID())
		{
